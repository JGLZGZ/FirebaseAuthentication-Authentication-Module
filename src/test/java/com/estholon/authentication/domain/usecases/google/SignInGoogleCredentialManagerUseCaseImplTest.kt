...
class SignInGoogleCredentialManagerUseCaseImplTest {

    private lateinit var repository: AuthenticationRepository
    private lateinit var sendEventUseCase: SendEventUseCase
    private lateinit var useCase: SignInGoogleCredentialManagerUseCaseImpl

    @Before
    fun setup() {
        repository = mockk()
        sendEventUseCase = mockk(relaxed = true)
        useCase = SignInGoogleCredentialManagerUseCaseImpl(repository, sendEventUseCase)
    }
    ...
    fun `invoke returns success and sends analytics`() = runTest {
        ...
        coEvery { repository.signInGoogleCredentialManager(activity) } returns Result.success(user)
        ...
        coVerify(exactly = 1) { repository.signInGoogleCredentialManager(activity) }
    ...
    fun `invoke returns failure and sends analytics`() = runTest {
        ...
        coEvery { repository.signInGoogleCredentialManager(activity) } returns Result.failure(exception)
