...
class LinkTwitterUseCaseImplTest {

    private lateinit var repository: AuthenticationRepository
    private lateinit var useCase: LinkTwitterUseCaseImpl

    @Before
    fun setup() {
        repository = mockk()
        useCase = LinkTwitterUseCaseImpl(repository)
    }
    ...
    fun `invoke returns result from repository`() = runTest {
        ...
        coEvery { repository.linkTwitter(activity) } returns Result.success(user)
